...
public class WriteLoops {
    ...
    public int driveHome() {
        int w = 0;
        String location = gpsCurrentLocation();

        // you need to use a .equals for two Strings.
        while(location.equals("Home") == false) {

            // calling
            driveSomeMore();
            w = w + 1;
            location = gpsCurrentLocation();
            // each time through the inner loop
        }
    ...
    public int checkGameScore() {
        ...
        int currentScore = gameNextScore(); // gameNextScore() returns a randaom value making the control of the loop impossible
        int runningScore = 0;
        ...
        // w is assigned randomly due to gameNextScore
        return w; // >= 3; Does not test for >=3 which would be a boolean
    }
